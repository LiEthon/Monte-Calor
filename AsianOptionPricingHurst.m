function [callPrice, putPrice, callPayoff, putPayoff, S,discountFactor] = AsianOptionPricing(S0, K, r, T, sigma, N, M, type,useControlVariate, hurst)
...
for i = 1:N
    ...
    % 利用GBM更新股票价格
    S(i+1, :) = S(i, :) .* exp((r - 0.5 * sigma^2) * dt + sigma * dW * hurst);
end
